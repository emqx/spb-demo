...
class DB:
    ...
    def create_tags_table(self):
        ...
        CREATE TABLE IF NOT EXISTS tag_values (
            `ts` TIMESTAMP, `tag` BINARY(128), `value` BINARY(128), `device` BINARY(128))
        """
        self.td.execute(sql)
    ...
    def insert_tag(self, device: str, tag: str, value: str, time: Timestamp):
        sql = f"INSERT INTO tag_values VALUES ('{time}', '{tag}', '{value}', '{device}')"
        logging.info(f"SQL: {sql}")
        result = self.td.execute(sql)
    ...
    def query_tag_range(self, device: str, tag: str, start: str, end: str) -> list[dict]:
        sql = f"SELECT * FROM tag_values WHERE device = '{device}' AND tag = '{tag}' AND ts > '{start}' AND ts < '{end}'"
